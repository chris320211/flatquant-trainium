import torch, math
# Using PyTorch kernels for Trainium compatibility (FP8 instead of Triton)
from deploy.kernels.pytorch.kron_matmul_pytorch import kron_matmul_pytorch as kron_matmul
from deploy.kernels.pytorch.block_matmul_pytorch import block_matmul_pytorch as block_matmul
import deploy
...
def _hadamard_matrix(n: int, device, dtype) -> torch.Tensor:
    """Build a normalised Hadamard matrix of size n (must be power-of-2) via recursive Sylvester construction."""
...
def _hadamard_transform_pytorch(x: torch.Tensor, scale: float = 1.0) -> torch.Tensor:
    """Pure-PyTorch Hadamard transform applied along the last dimension."""
    n = x.shape[-1]
...
def matmul_hadU_cuda(X, hadK, K):
    n = X.shape[-1]
    had_fn = _fht.hadamard_transform if _HAS_FAST_HAD else _hadamard_transform_pytorch
    scale = 1.0 / (n ** 0.5)

    if K == 1:
        return had_fn(X.contiguous(), scale)
    input = X.view(*X.shape[:-1], K, n // K)
    input = had_fn(input.contiguous(), scale)
