"""
Pure PyTorch implementation of Kronecker-style matrix multiplication with FP8 quantization.
...
def quantize_fp8(x, clip_factor_max, clip_factor_min):
    ...
    # Quantize to FP8
    # PyTorch FP8 dtypes: torch.float8_e4m3fn (4-bit exponent, 3-bit mantissa)
    # or torch.float8_e5m2 (5-bit exponent, 2-bit mantissa)
    # Using e4m3fn which is better for activations/forward pass
    x_scaled = x / scale
...
def kron_matmul_pytorch(a, b, c, seq_len, clip_factor_a_max, clip_factor_a_min):
    """
    Performs Kronecker-style matrix multiplication: a @ b @ c with FP8 quantization.

    Pure PyTorch implementation compatible with Trainium and optimized for performance.

    Args:
        a: [M, M] matrix - left transformation matrix
        b: [B, M, N] batch of matrices - input activations
        c: [N, N] matrix - right transformation matrix
        seq_len: Sequence length for batch processing (used for organizational purposes)
    ...
    Returns:
        PackedQuantizedTensor with FP8 quantized result of a @ b @ c
    ...
    assert a.shape[1] == b.shape[1], f"Incompatible dimensions: a.shape[1]={a.shape[1]}, b.shape[1]={b.shape[1]}"
    ...
    # Compute a @ b @ c using efficient PyTorch batched operations
    # Step 1: a @ b where a is [M, M] and b is [B, M, N]
    # We can use torch.bmm by expanding a: [1, M, M] broadcast with [B, M, N]
    # Or use @ which handles broadcasting: [M, M] @ [B, M, N] -> [B, M, N]
    ab = torch.matmul(a, b)  # Broadcasting: [M, M] @ [B, M, N] -> [B, M, N]

    # Step 2: (a @ b) @ c where ab is [B, M, N] and c is [N, N]
    # [B, M, N] @ [N, N] -> [B, M, N]
    abc = torch.matmul(ab, c)  # [B, M, N]

    # Quantize result to FP8 with learned clipping
    return quantize_fp8(abc, clip_factor_a_max, clip_factor_a_min)
...
def benchmark(B, M, N, S, provider):
    ...
    if provider == 'pytorch':
        ...
        # Warmup
        for _ in range(10):
            _ = kron_matmul_pytorch(a, b, c, S, clip_factor_a_max, clip_factor_a_min)
        ...
        # Compute TFLOPS
        # Operations: a @ b (B * M * M * N) + ab @ c (B * M * N * N)
        # Each matmul is 2*mnk FLOPs
        total_flops = 2 * B * (M * M * N + M * N * N)
