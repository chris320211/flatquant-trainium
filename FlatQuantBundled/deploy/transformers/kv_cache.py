...
try:
    from fast_hadamard_transform import hadamard_transform as _fht_hadamard
except ImportError:
    _fht_hadamard = None
...
def matmul_had_cuda(X, dtype):
    n = X.shape[-1]
    if _fht_hadamard is not None:
        out = _fht_hadamard(X.to(dtype).contiguous(), scale=1 / math.sqrt(n))
    else:
        from deploy.functional.online_trans import _hadamard_transform_pytorch
        out = _hadamard_transform_pytorch(X.to(dtype).contiguous(), scale=1 / math.sqrt(n))
    return out.to(X.dtype).view(X.shape)


def _init_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                     last_page_offset, k, v, k_param, v_param,
                     seqlen_indptr, layer_idx):
    """Place initial KV states into the paged structure (pure PyTorch)."""
    page_size = kv_data.shape[4]
    n_batches = len(seqlen_indptr) - 1
    for b in range(n_batches):
        seq_start = seqlen_indptr[b].item()
        seq_end = seqlen_indptr[b + 1].item()
        page_base = kv_indptr[b].item()
        for t in range(seq_end - seq_start):
            phys_page = kv_indices[page_base + t // page_size].item()
            pos = t % page_size
            kv_data[phys_page, layer_idx, 0, :, pos, :] = k[seq_start + t]
            kv_data[phys_page, layer_idx, 1, :, pos, :] = v[seq_start + t]
            kv_param[phys_page, layer_idx, 0, :, pos, :] = k_param[seq_start + t]
            kv_param[phys_page, layer_idx, 1, :, pos, :] = v_param[seq_start + t]


def _append_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                       last_page_offset, k, v, k_param, v_param,
                       layer_idx):
    """Append one token per batch to paged KV (pure PyTorch)."""
    batch_size = k.shape[0]
    page_size = kv_data.shape[4]
    for b in range(batch_size):
        pos = last_page_offset[b].item()
        if pos == page_size:
            pos = 0
        last_page_logical = kv_indptr[b + 1].item() - 1
        phys_page = kv_indices[last_page_logical].item()
        kv_data[phys_page, layer_idx, 0, :, pos, :] = k[b]
        kv_data[phys_page, layer_idx, 1, :, pos, :] = v[b]
        kv_param[phys_page, layer_idx, 0, :, pos, :] = k_param[b]
        kv_param[phys_page, layer_idx, 1, :, pos, :] = v_param[b]


def _batch_decode_pytorch(o, q, kv_data, kv_param, kv_indptr, kv_indices,
                          last_page_offset, layer_idx, *, quantized):
    """Compute single-token attention over paged KV cache (pure PyTorch)."""
    ...
    for b in range(batch_size):
        ...
        for pi in range(page_start, page_end):
            phys = kv_indices[pi].item()
            valid = last_page_offset[b].item() if pi == page_end - 1 else page_size
            kp = kv_data[phys, layer_idx, 0, :, :valid, :]
            vp = kv_data[phys, layer_idx, 1, :, :valid, :]
            kp_param = kv_param[phys, layer_idx, 0, :, :valid, :]
            vp_param = kv_param[phys, layer_idx, 1, :, :valid, :]
            if quantized:
                kp = unpack_i4_and_asym_dequantize(kp, kp_param[..., 0:1], kp_param[..., 1:2])
                vp = unpack_i4_and_asym_dequantize(vp, vp_param[..., 0:1], vp_param[..., 1:2])
            k_pages.append(kp)
            v_pages.append(vp)

        k_full = torch.cat(k_pages, dim=1).to(q.dtype)
        v_full = torch.cat(v_pages, dim=1).to(q.dtype)
        q_b = q[b].unsqueeze(1)
        scale = head_dim ** -0.5
        attn = torch.matmul(q_b, k_full.transpose(-1, -2)) * scale
        attn = torch.softmax(attn, dim=-1)
        o[b] = torch.matmul(attn, v_full).squeeze(1)
...
def init_kv_i4(kv_data, kv_param, kv_indptr, kv_indices,
    ...
    if _CUDA is not None:
        return _CUDA.init_kv_i4(kv_data, kv_param, kv_indptr, kv_indices,
                                last_page_offset, k, v, k_param, v_param,
                                seqlen_indptr, layer_idx)
    return _init_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                            last_page_offset, k, v, k_param, v_param,
                            seqlen_indptr, layer_idx)


def append_kv_i4(kv_data, kv_param, kv_indptr, kv_indices,
                 last_page_offset, k, v, k_param, v_param, layer_idx):
    if _CUDA is not None:
        return _CUDA.append_kv_i4(kv_data, kv_param, kv_indptr, kv_indices,
                                  last_page_offset, k, v, k_param, v_param,
                                  layer_idx)
    return _append_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                              last_page_offset, k, v, k_param, v_param,
                              layer_idx)
...
def batch_decode_i4(o, q, kv_data, kv_param, kv_indptr, kv_indices,
    ...
    if _CUDA is not None:
        return _CUDA.batch_decode_i4(o, q, kv_data, kv_param, kv_indptr,
                                     kv_indices, last_page_offset, layer_idx)
    return _batch_decode_pytorch(o, q, kv_data, kv_param, kv_indptr, kv_indices,
                                 last_page_offset, layer_idx, quantized=True)
...
def init_kv_f16(kv_data, kv_param, kv_indptr, kv_indices,
                ...
                seqlen_indptr, layer_idx):
    if _CUDA is not None:
        return _CUDA.init_kv_f16(kv_data, kv_param, kv_indptr, kv_indices,
                                 last_page_offset, k, v, k_param, v_param,
                                 seqlen_indptr, layer_idx)
    return _init_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                            last_page_offset, k, v, k_param, v_param,
...
def append_kv_f16(kv_data, kv_param, kv_indptr, kv_indices,
                  last_page_offset, k, v, k_param, v_param, layer_idx):
    if _CUDA is not None:
        return _CUDA.append_kv_f16(kv_data, kv_param, kv_indptr, kv_indices,
                                   last_page_offset, k, v, k_param, v_param,
                                   layer_idx)
    return _append_kv_pytorch(kv_data, kv_param, kv_indptr, kv_indices,
                              last_page_offset, k, v, k_param, v_param,
...
def batch_decode_f16(o, q, kv_data, kv_param, kv_indptr, kv_indices,
                     last_page_offset, layer_idx):
    if _CUDA is not None:
        return _CUDA.batch_decode_f16(o, q, kv_data, kv_param, kv_indptr,
                                      kv_indices, last_page_offset, layer_idx)
    return _batch_decode_pytorch(o, q, kv_data, kv_param, kv_indptr, kv_indices,
                                 last_page_offset, layer_idx, quantized=False)
