...
def _portable_attention(query, key, value, query_length, attention_mask=None, is_causal=True, **kwargs):
    """
    PyTorch SDPA fallback — works on CPU, CUDA, and Trainium (XLA).
    Expects shapes: [bsz, seq, heads, head_dim].
    """
    q = query.transpose(1, 2)
    k = key.transpose(1, 2)
    v = value.transpose(1, 2)
    out = _F.scaled_dot_product_attention(q, k, v, attn_mask=None, is_causal=is_causal)
        ...
        **kwargs,
        ...
        if isinstance(cache_out, tuple):
            ...
            if trans_mat_for_q is not None:
                query_states = torch.matmul(query_states.to(trans_mat_for_q.dtype), trans_mat_for_q)
            _attn_fn = _flash_attention_forward if _HAS_FLASH_ATTN else _portable_attention
            attn_output = _attn_fn(
                query_states,
                key_states,
                value_states,
                query_length=q_len,
